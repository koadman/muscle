...
// Special case for i=uLengthA
	...
	if( uLengthA > 1 )
		...
		{
		// AED 7/1/2006: handle the case when an input seq has a single nt without
		// crashing.  unsure whether this modified score calculation is "correct"
		MCurr[1] = ScoreProfPos2(PA[uLengthA-1], PB[0]) + (0)*e +
		  PA[0].m_scoreGapOpen + PA[0].m_scoreGapClose;
		}
